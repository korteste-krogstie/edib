...
std::ostream& operator<<(std::ostream& os, const period_t& p) {
	os << p.to_string();
	return os;
}

std::string period_t::to_string() const {
	static const cal_t utc;
	return utc.to_string(*this);
}

std::string cal_t::to_string(time_t t) const {
	char s[100];
	switch(t) {
		case no_time:std::sprintf(s, "no_time"); break;
		case min_time:std::sprintf(s, "-oo"); break;
		case max_time:std::sprintf(s, "+oo"); break;
		default: {
			...
			if(tz) {
				if(tz_minutes)
					std::sprintf(tzs,"%+03d:%02d",tz_hours,tz_minutes);
				else
					std::sprintf(tzs,"%+03d",tz_hours);
			} else {
				std::strcpy(tzs,"Z");
			}
			std::sprintf(s, "%04d-%02d-%02dT%02d:%02d:%02d%s", c.year, c.month, c.day, c.hour, c.minute, c.second,tzs);
			break;
		}
	}
	return s;
...
std::string cal_t::to_string(period_t p) const {
	if(p.valid()) {
		return std::string("[") + to_string(p.start) + "," + to_string(p.end) + ">";
	}
	return "[not-valid-period>";
}

time_t cal_t::time(YMDhms c) const {
	if(c.is_null()) return no_time;
	if(c==YMDhms::max()) return max_time;
	if(c==YMDhms::min()) return min_time;
	if (!c.is_valid_coordinates())
		throw std::runtime_error("Calendar.time with invalid YMDhms coordinates attempted");

	time_t r= ((int(day_number(c)) - UnixDay)*DAY) + seconds(c.hour, c.minute, c.second);
...
YMDhms  cal_t::calendar_units(time_t t) const {
	if (t == no_time)  return YMDhms();
	if (t == max_time) return YMDhms::max();
	if (t == min_time) return YMDhms::min();
	...
	time_t tj = UnixSecond + t;
	time_t td = DAY*(tj / DAY);
	timespan_t dx = tj - td;// n seconds this day.
	r.hour = int(dx / HOUR);
	dx -= r.hour*HOUR;
...
int cal_t::day_of_week(time_t t) const {
	if (t == no_time || t == min_time || t == max_time)
		return -1;
...
size_t cal_t::day_of_year(time_t t) const {
	if (t == no_time || t == max_time || t == min_time) return -1;
	auto x = calendar_units(t );
	YMDhms y(x.year, 1, 1);
	return 1 + day_number(x) - day_number(y);
}
///< returns the month of t, 1..12, -1 of not valid time
int cal_t::month(time_t t) const {
	if (t == no_time || t == max_time || t == min_time) return -1;
	return calendar_units(t ).month;
}

time_t cal_t::trim(time_t t, timespan_t deltaT) const {
	if (t == no_time || t == min_time || t == max_time || deltaT==timespan_t(0))
		return t;
	switch (deltaT) {
	case YEAR: {
		auto c = calendar_units(t);
	...
	const time_t t0 = time_t(3LL * DAY) + time_t(WEEK * 52L * 2000LL);// + 3 days to get to a isoweek monday, then add 2000 years to get a positive number
	t += t0 + tz_offset;
	t= deltaT*(t / deltaT) - t0 ;
	return  t - tz_info->utc_offset(t);
}

time_t cal_t::add(time_t t, timespan_t deltaT, long n) const {
	auto dt=n*deltaT;
	switch (deltaT) {
		case YEAR: {
			auto c=calendar_units(t);
...
timespan_t cal_t::diff_units(time_t t1, time_t t2, timespan_t deltaT, timespan_t &remainder) const {
	if (t1 == no_time || t2 == no_time || deltaT == 0L) {
		remainder = 0L;
		return 0L;
	}
	int sgn=1;
	if(t1>t2){sgn=-1;std::swap(t1,t2);}
	timespan_t n_units= (t2-t1)/deltaT;// a *rough* estimate in case of dst or month/year
	...
	switch (deltaT) {
		case cal_t::MONTH: n_units -= n_units/72; // MONTH is 30 days, a real one  is ~ 30.41, after 72 monts this adds up to ~ 1 month error
		case cal_t::YEAR:
		case cal_t::WEEK:
		case cal_t::DAY:{
			...
			if(t2x>t2) { // got one step to far
				...
				remainder=timespan_t(0);// perfect hit, zero remainder
			}
		} break;
		default: {
...
namespace zone {
	...
	struct boost_tz_info { // limitation is posix time, def from 1901 and onward
		ptime posix_t1970;///< epoch reference in ptime, needed to convert to time_t
		time_zone_ptr tzinfo;///< we extract boost info from this one
		/**\brief convert a ptime to a time_t */
		time_t to_time_t(ptime p) const { return (p-posix_t1970).total_seconds();}

		boost_tz_info(time_zone_ptr tzinfo):posix_t1970((date(1970,1,1))),tzinfo(tzinfo) {}


		time_t dst_start(int year) const {
			return to_time_t(tzinfo->dst_local_start_time(year)) - tzinfo->base_utc_offset().total_seconds();
		}
		time_t dst_end(int year) const {
			return to_time_t(tzinfo->dst_local_end_time(year)) - tzinfo->base_utc_offset().total_seconds() - tzinfo->dst_offset().total_seconds();
		}
		timespan_t base_offset() const {
			return timespan_t(tzinfo->base_utc_offset().total_seconds());
		}
		timespan_t dst_offset(int y) const {
			return timespan_t(tzinfo->dst_offset().total_seconds());
		}
	...
	static tz_region_def tzdef[]={ // as defined in boost 1.60 date_zonespec.csv 2016.01.30
...
cal_t::cal_t(std::string region_id) {
	for(size_t i=0;i<zone::n_tzdef;++i) {
		if(region_id==zone::tzdef[i].region) {
			tz_info=zone::create_from_posix_definition(zone::tzdef[i].posix_definition);
		}
	}
	if(!tz_info)
		throw std::runtime_error(std::string("time zone region id '")+region_id+std::string("' not found, use .region_id_list() to get configured time zones"));
}

std::vector<std::string> cal_t::region_id_list() {
	std::vector<std::string> r;
	for(size_t i=0;i<zone::n_tzdef;++i) r.push_back(zone::tzdef[i].region);
	return r;
}
